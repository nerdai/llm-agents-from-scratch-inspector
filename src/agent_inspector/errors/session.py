...
class StepExecutionError(SessionServiceError):
    ...
    propagates -- a genuine LLM/framework-level failure (e.g. a
    network failure talking to the backbone LLM). A tool call itself
    raising does *not* reach here: ``llm-agents-from-scratch>=0.0.22``
    catches every tool call's exception internally (concurrent
    execution via ``asyncio.gather`` needs to, so one tool failing
    doesn't crash the others), and
    ``_RecordingSyncTool``/``_RecordingAsyncTool`` (services/session.py)
    catch it even earlier, turning it into an ordinary
    ``ToolCallResult(error=True, ...)`` -- same as a tool the
    framework can't find by name, or one that reports its own failure
    directly. Route layer should map this to ``502``.
    """
